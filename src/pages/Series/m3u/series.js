...
const Series = () => {
  ...
  const allSeriesCategories = path.state?.usr?.seriesCategories;

  const arrangedCategory = arrangeByGroupTitle(allSeriesCategories);
  ...
  const redirectToSeriesInfo = (data) => {
    navigate("/dashboard/m3u/series-info", { state: { info: data } });
  };

  return (
    <div>
      <Navbar notCategories={true} heading="Series" />
      {allSeriesCategories.length > 0 ? (
        <div className="flex flex-row gap-5 md:mx-5 mx-auto">
          ...
          >
            {allCategory?.map((series, index) => {
              return (
                <NewMoviesSeriesCard
                  data={series}
                  key={index}
                  onClick={redirectToSeriesInfo}
                />
              );
            })}
          </div>
